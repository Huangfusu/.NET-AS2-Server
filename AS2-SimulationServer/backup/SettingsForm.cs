...
namespace AS2_SimulationServer
{
    public partial class SettingsForm : Form
    {
        ...
        {
            ThreadPool.SetMinThreads(300, Settings.DefaultCompletionPortThread);
            ThreadPool.SetMaxThreads(300, Settings.DefaultCompletionPortThread);
                    ...
                    if (!String.IsNullOrEmpty(Settings.AvgFolder))
                    {
                        if (!e.Cancel)
                        {
                            Task.Factory.StartNew(() =>
                            {
                                ...
                                Parallel.For(0, Settings.AvgMessagePerMin, (i, loopSate) =>
                                {
                                    if (e.Cancel)
                                        loopSate.Break();
                    ...
                    if (!String.IsNullOrEmpty(Settings.MinFolder))
                    {
                        if (!e.Cancel)
                        {
                            Task.Factory.StartNew(() =>
                            {
                                ...
                                Parallel.For(0, Settings.MinMessagePerMin, (i, loopSate) =>
                                {
                                    if (e.Cancel)
                                        loopSate.Break();
                                    for (int j = 0; j < urlLis.Count(); j ++)
                    ...
                    if (!String.IsNullOrEmpty(Settings.MaxFolder))
                    {
                        if (!e.Cancel)
                        {
                            Task.Factory.StartNew(() =>
                            {
                                ...
                                Parallel.For(0, Settings.MaxMessagePerMin, (i,loopSate) =>
                                {
                                    if (e.Cancel)
                                        loopSate.Break();
                                    for (int j = 0; j < urlLis.Count(); j ++)
        ...
        {
           LblDuration.Text = watch.Elapsed.ToString(@"dd\d\:\ hh\h\:\ mm\m\:\ ss\s");
        }
